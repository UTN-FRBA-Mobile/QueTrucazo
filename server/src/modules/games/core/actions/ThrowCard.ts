...
import { Game, GameStep } from "../domain/Game";
...
export class ThrowCard {
    ...
    public async invoke(id: Game['id'], userId: UserId, card: Card, step: GameStep): Promise<void> {
        const game = await this.gameService.getUserGame(id, userId);
        const updatedGame = game.throwCard(userId, card, step);
